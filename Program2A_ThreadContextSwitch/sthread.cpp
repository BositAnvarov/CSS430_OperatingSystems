...
/*
  - This function is called from sthread_init() and sthread_yield
  - Capture the current threads jmp_env and activation record 
  - capture stack pointer and base pointer using assemly
  - get the size using sp - bp
  - allocate new space in heap to save the activation record'
  - copy from functions stack to the heap space 
  - put the current thread inside the queue
*/
#define capture( )                \
{							\
...
/*
  - sthread voluntarily yields the CPU
  - if the alarmed is on, then set it off and continue
  - memorize the thread conext with setjmp and caputure it
  - use longjump for scheduler
  - use memcpy to copy back this threads contents from back to stack
*/
